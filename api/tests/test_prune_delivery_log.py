...
_TEST_DELIVERY_LOG_CREATION_TIMES: list[datetime] = [
...
def _delivery_logs():
    for created_at in _TEST_DELIVERY_LOG_CREATION_TIMES:
        with freeze_time(created_at):
            DeliveryLogFactory.create()
...
def test_valid_months(
    ...
    """
    Test that the prune_delivery_log command deletes the expected number of logs
    based on the months and dry_run arguments, and the correct message is printed.
    """
    orig_count = 10
    assert DeliveryLog.objects.count() == orig_count
    with freeze_time(_TEST_TIME):
        kwargs = {"dry_run": dry_run}
        if months is not None:
            kwargs["months"] = months
        call_command("prune_delivery_log", **kwargs)
    captured = capsys.readouterr()
    dry_run_msg = "Running in dry-run mode i.e. not committing changes!\n"
    del_msg = (
        f"Deleted {expected_deletion_count} delivery logs "
        f"created at least {6 if months is None else months} months ago"
    )
    assert captured.err == ""
    ...
    if dry_run:
        ...
        assert DeliveryLog.objects.count() == orig_count - expected_deletion_count


@pytest.mark.django_db
def test_invalid_months(capsys):
    """
    Test that the prune_delivery_log command raises a ValueError if the months argument
    is negative, and that no logs are deleted.
    """
    orig_delivery_log_count = DeliveryLog.objects.count()
    assert orig_delivery_log_count
    kwargs = {"months": -1}
    ...
    assert DeliveryLog.objects.count() == orig_delivery_log_count
